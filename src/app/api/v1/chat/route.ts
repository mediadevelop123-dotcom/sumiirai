...
- テンプレートに「Example_Input / Example_Output」が含まれる場合は、**Example_Outputをそのまま最初に出力**して「このように作成します」と1行添える。続けて、最も重要な1つの情報だけを自然な1文で聞く（例：「返信したいメッセージを貼り付けていただければ、実際の返信文を作ります」）。「〇〇：【ここに記入】」のような複数フィールドを並べた入力フォーム形式を提示しない
