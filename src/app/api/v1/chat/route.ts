...
- テンプレートに「Example_Input / Example_Output」が含まれる場合は、Example_Inputを使って**トーンや媒体を変えた2パターン（例：カジュアル版とビジネス版）**のサンプル出力を最初に見せる。「このように媒体・トーンに合わせて作成します」と1行添え、最後に「返信したいメッセージ、トーン、媒体を教えていただければ作成します」と自然な1文で締める。「〇〇：【ここに記入】」のような複数フィールドを並べた入力フォーム形式を提示しない
